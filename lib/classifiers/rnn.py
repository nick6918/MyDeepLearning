...
class CaptioningRNN(object):
    ...
    def __init__(self, word_to_idx, input_dim=512, wordvec_dim=128,
                 hidden_dim=128, cell_type='rnn', dtype=np.float32, reg=0.0):
        """
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type == "rnn":
            ...
            dLoss_dx, dLoss_dh0, grads["Wx"], grads["Wh"], grads["b"] = lstm_backward(dLoss_dh, cache3)
        grads["Wx"] += reg*Wx
        grads["Wh"] += reg*Wh
    ...
    def sample(self, features, max_length=30):
        ...
        W_vocab, b_vocab = self.params['W_vocab'], self.params['b_vocab']
x
        # Implement test-time sampling for the model and initialize the hidden state 
        # of the RNN by applying the learned affine transform to the input image features.
        # The RNN should be the <START> token; its value is stored in variable self._start. 
        # At each timestep:        
        ...
        prev_h = h0
        next_c = np.zeros(prev_h.shape)
        for i in range(max_length):
            prev_word_embeded, _ = word_embedding_forward(prev_word, W_embed) #(N, D)
            if self.cell_type == "rnn":
                next_h, _ = rnn_step_forward(prev_word_embeded, prev_h, Wx, Wh, b) #(N, H)
            else:
                next_h, next_c, _ = lstm_step_forward(prev_word_embeded, prev_h, next_c, Wx, Wh, b) #(N, H)
            h_score, _ = affine_forward(next_h, W_vocab, b_vocab) #(N, V)
            next_word = np.argmax(h_score, axis=1) #(N, )
