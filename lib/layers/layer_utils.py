...
def conv_bn_relu_pool_forward(x, w, b, gamma, beta, conv_param, bn_param, pool_param):
    H1, cache1 = conv_bn_relu_forward(x, w, b, gamma, beta, conv_param, bn_param)
    out, cache2 = max_pool_forward_fast(H1, pool_param)
    cache = (cache1, cache2)
    return out, cache

def conv_bn_relu_pool_backward(dout, cache):
    cache_conv, cache_pool = cache
    dH1 = max_pool_backward_fast(dout, cache_pool)
    dx, dw, db, dgamma, dbeta = conv_bn_relu_backward(dH1, cache_conv)
    return dx, dw, db, dgamma, dbeta
